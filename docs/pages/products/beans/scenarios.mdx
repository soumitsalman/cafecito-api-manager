---
title: Beans API Workflows
description: Compose Beans Article, Source, Story, and discovery routes into publisher-content workflows, including news, earnings, and litigation monitoring.
---
...
| Send only documented parameters. | Unknown or route-inapplicable keys, including `content_type` on top-headlines and the news feed shortcuts, and `from`/`to` on feeds, return HTTP `400`. |
