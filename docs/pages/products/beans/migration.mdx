---
description: Migrate your news API stack to Beans API. Map TheNewsAPI, World News API, GNews, finlight, NewsAPI.ai, and NewsData.io workflows to Beans Articles, Sources, Stories, and feeds.
---
...
| A newest-first **news** feed | `GET /beans/news/latest` | News-scoped shortcut equivalent to latest with `content_type=news`. Does not accept `content_type`, `ids`, `urls`, `from`, or `to`. |
...
| Attention-ranked **news** | `GET /beans/news/trending` | News-scoped shortcut equivalent to trending with `content_type=news`. Does not accept `content_type`, `ids`, `urls`, `from`, or `to`. |
...
| Build a newest-first news feed | `GET /beans/news/latest` -> follow `pagination.next_cursor` |
| Build a headline or attention feed | `GET /beans/news/top-headlines` or `GET /beans/articles/trending` |
| Build an attention-ranked news feed | `GET /beans/news/trending` -> follow `pagination.next_cursor` |
...
| Language | `languages` | CSV ISO 639 language codes; any listed value matches. A stored language matches if it equals a listed code or starts with that code. |
| Publication bounds | `from` and `to` | Inclusive `YYYY-MM-DD` bounds on search, similar, Stories, Story Articles, and mention `observed_at`. Not accepted on latest, trending, top-headlines, or the news feed shortcuts. |
